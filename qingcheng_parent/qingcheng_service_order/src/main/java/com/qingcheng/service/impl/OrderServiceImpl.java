...
public class OrderServiceImpl implements OrderService {
    ...
    public Map<String, Object> add(Order order) {
        ...
        IntStream numStream = orderItems.stream().mapToInt(cart -> cart.getNum());
        int totalNum = numStream.sum();//总数量

        IntStream moneyStream = orderItems.stream().mapToInt(cart -> cart.getMoney());
        int totalMoney = moneyStream.sum();//总金额

        int preMoney = cartService.preferential(order.getUsername());//满减优惠金额

        order.setPostFee(0);//邮费
        order.setIsDelete("0");//未删除
        order.setTotalNum(totalNum);//数量
        order.setTotalMoney(totalMoney);//金额

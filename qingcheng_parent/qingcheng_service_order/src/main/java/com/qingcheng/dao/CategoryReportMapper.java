...
public interface CategoryReportMapper extends Mapper<CategoryReport> {
    ...
    @Select("SELECT category_id1 categoryId1,v.name categoryName, SUM(num) num , SUM(money) money  " +
            "FROM `tb_category_report` r,v_category1 v  " +
            "WHERE r.category_id1 = v.id and count_date>=#{date1} AND count_date<=#{date2}  " +

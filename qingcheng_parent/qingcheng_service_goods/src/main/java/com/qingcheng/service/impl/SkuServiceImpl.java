...
public class SkuServiceImpl implements SkuService {
    ...
    public void saveAllPriceToRedis() {
        ...
        if (!redisTemplate.hasKey(CacheKey.SKU_PRICE)) {
            ...
            System.out.println("缓存中已经有价格数据啦！");
        }
    }

    public Integer findPrice(String id) {

        //如果缓存为空，从数据库中查询价格，并将价格存入缓存中
        if(redisTemplate.boundHashOps(CacheKey.SKU_PRICE).get(id)==null||"".equals(redisTemplate.boundHashOps(CacheKey.SKU_PRICE).get(id))){
            System.out.println("我从数据库中查询价格了");
            Sku sku = skuMapper.selectByPrimaryKey(id);
            //将查询到的价格存入redis
            redisTemplate.boundHashOps(CacheKey.SKU_PRICE).put(id,sku.getPrice());
            return sku.getPrice();
        };

        System.out.println("我从缓存中读取的价格");
        //缓存不为空,从缓存中查询价格

...
public class CategoryServiceImpl implements CategoryService {
    ...
    public void delete(Integer id) {
        //判断当前分类是否存在下级，如果存在下级就不允许删除
        Example example = new Example(Category.class);
        Example.Criteria criteria = example.createCriteria();
        criteria.andEqualTo("parentId",id);
        int count = categoryMapper.selectCountByExample(example);
        if(count > 0){
            throw new RuntimeException("当前分类存在下级分类，不能删除");
        }
    ...
    public List<Map> findCategoryTree() {
        ...
        //从缓存中提取分类，如果缓存总没有，从数据库中查
        if(redisTemplate.boundValueOps(CacheKey.CATEGORY_TRUE).get()!=null){
            System.out.println("从缓存中提取");
            return (List<Map>) redisTemplate.boundValueOps(CacheKey.CATEGORY_TRUE).get();
        }else{
            Example example = new Example(Category.class);
            Example.Criteria criteria = example.createCriteria();
    ...
    public void saveCateGoryTreeToRedis() {
        ...
        if(!redisTemplate.hasKey(CacheKey.CATEGORY_TRUE)){
            Example example = new Example(Category.class);
            Example.Criteria criteria = example.createCriteria();
            criteria.andEqualTo("isShow", "1");//是否显示
            example.setOrderByClause("seq");//排序
            ...
            System.out.println(categoryList+"11111111111111111111111111111111111");

            List<Map> cateGortTreeList = findByParentId(categoryList, 0);
            //存入redis
            redisTemplate.boundValueOps(CacheKey.CATEGORY_TRUE).set(cateGortTreeList);
            System.out.println("我将广告存到缓存中了");
        }else{
            System.out.println("我从缓存中取数据");
        }
